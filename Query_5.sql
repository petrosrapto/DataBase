...
SELECT pf1.field_name AS first_field, pf2.field_name AS second_field, count(*) AS projects_per_field_tuple FROM pf pf1 
INNER JOIN pf pf2 
ON pf1.proj_id = pf2.proj_id
WHERE pf1.field_id < pf2.field_id  /* Skipping Duplicates */
GROUP BY pf1.field_name, pf2.field_name
ORDER BY projects_per_field_tuple DESC
LIMIT 3;